...
class WeatherAPI:
    ...
    def GetWeatherByPosition(self, lat: float, lon: float) -> str:
        data = get(self.__weather_current_url, params={"lat": f"{lat}", "units": "metric", "lon": f"{lon}", 'lang': 'ru', "APPID": Settings.APPID_OW}).json()
        ...
        return f"{weather_description} -> Температура: {weather_temp}°K | {round(float(weather_temp) - 273, 2)}°C -> Давление: {round(float(weather_pressure) * 100 / 133, 2)}мм. рт. ст. -> Влажность: {weather_humidity}% -> Ветер: {weather_wind_speed}м/с" #  , {weather_wind_deg}°
